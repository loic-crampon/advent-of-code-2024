        ...
        if (len(values) > 1):
            for point_a in range(len(values) - 1):
                for point_b in range(point_a + 1, len(values)):
                    ...
                    while (antiniode_a[0] >= 0) and (antiniode_a[0] < width_map) and (antiniode_a[1] >= 0) and (antiniode_a[1] < height_map):
                        if antiniode_a not in antinodes:
                            antinodes.append(antiniode_a)
                        antiniode_a = (antiniode_a[0] + dist_x, antiniode_a[1] + dist_y)

                    # Add antenna position itself
                    if values[point_a] not in antinodes:
                        antinodes.append(values[point_a])

                    antiniode_b = (values[point_b][0] - dist_x, values[point_b][1] - dist_y)
                    while (antiniode_b[0] >= 0) and (antiniode_b[0] < width_map) and (antiniode_b[1] >= 0) and (antiniode_b[1] < height_map):
                        if antiniode_b not in antinodes:
                            antinodes.append(antiniode_b)
                        antiniode_b = (antiniode_b[0] - dist_x, antiniode_b[1] - dist_y)

                    # Add antenna position itself
                    if values[point_b] not in antinodes:
                        antinodes.append(values[point_b])
    return antinodes
